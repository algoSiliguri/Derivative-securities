...
class OptionStrategyBuilder:
    ...
    def __get_SPX_data_on_expiry(self):
        ...
        # print(df.Date)
        # print(df.loc[df.Date=='2015-09-11','Close'])
    ...
    def ironcondor(self):
        ...
        S = np.arange(self.spot*0.8, self.spot*1.2, self.spot *0.01)  # Get a list of spot prices
        ...
        S = np.arange(list(close)[0], list(close)[0]+0.01,1)
        E1, E2 = self.__calc_monthly_vol()
        Price1 = self.__get_strikes_option_prices(E1, 0)  
        Price2 = self.__get_strikes_option_prices(E2, 1)
        ...
        cost = (-1*Price1, -1*Price2, Price3, Price4, -1*Price1+-1*Price2+Price3+Price4)
        value_on_expiry = (P_1[0], P_2[0], P_3[0], P_4[0], P_1[0]+P_2[0]+P_3[0]+P_4[0])
        PnL = P_1[0] + P_2[0] + P_3[0] + P_4[0]

        return (cost, value_on_expiry, PnL)